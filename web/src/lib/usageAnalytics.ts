...
/** Window lengths (hours) for the summary distribution table. */
export const SUMMARY_WINDOW_HOURS = [1, 2, 3, 6, 12, 24] as const
export type SummaryWindowHours = (typeof SUMMARY_WINDOW_HOURS)[number]

export interface RollingWindowSummaryRow {
  ...
  p95: number
  max: number
  /** Number of sliding-window samples (starts + prorated long intervals). */
  sampleCount: number
  /** Time range where `max` kWh occurred (one representative window if tied). Billing-local instants. */
  maxWindowStartMs: number | null
  maxWindowEndMs: number | null
}
...
export interface PeriodUsageAnalytics {
  periodId: string
  periodLabel: string
  /** Per window length: distribution of kWh over every valid sliding window (same rules as prior max). */
  rollingWindowSummary: RollingWindowSummaryRow[]
...
/** Sliding-window kWh plus the interval span that produced each total (for max-window tooltips). */
function allRollingKwhSamplesInRun(run: UsageInterval[], windowMs: number): RollingWindowSample[] {
  if (run.length === 0) return []
  const n = run.length
  const pref = new Array<number>(n + 1).fill(0)
  for (let i = 0; i < n; i++) pref[i + 1] = pref[i]! + run[i]!.kWh

  const out: RollingWindowSample[] = []
  for (let i = 0; i < n; i++) {
    const t0 = run[i]!.startMs
    let lo = i
    let hi = n - 1
    ...
    if (bestJ >= i) {
      out.push({
        kwh: pref[bestJ + 1]! - pref[i]!,
        windowStartMs: t0,
        windowEndMs: run[bestJ]!.endMs,
      })
    }
  }
  return out
...
/**
 * All sliding-window samples for `windowHours` (contiguous runs, each start index) plus
 * prorated samples for single intervals longer than the window.
 */
function allRollingKwhSamples(intervals: UsageInterval[], windowHours: number): RollingWindowSample[] {
  const windowMs = windowHours * 3600 * 1000
  const runs = splitContiguousRuns(intervals)
  const out: RollingWindowSample[] = []
  for (const run of runs) {
    out.push(...allRollingKwhSamplesInRun(run, windowMs))
  }
  for (const iv of intervals) {
    const dur = iv.endMs - iv.startMs
    if (dur > windowMs && dur > 0) {
      out.push({
        kwh: iv.kWh * (windowMs / dur),
        windowStartMs: iv.startMs,
        windowEndMs: iv.startMs + windowMs,
      })
    }
  }
  return out
...
function rollingWindowSummaryStats(values: number[]): Omit<
  RollingWindowSummaryRow,
...
function rollingWindowSummaryFromSamples(
  samples: RollingWindowSample[],
  windowHours: SummaryWindowHours,
  label: string,
): RollingWindowSummaryRow {
  if (samples.length === 0) {
    return {
      ...
      max: 0,
      sampleCount: 0,
      maxWindowStartMs: null,
      maxWindowEndMs: null,
    }
  }
  const values = samples.map((s) => s.kwh)
  const stats = rollingWindowSummaryStats(values)
  let best = 0
  for (let i = 1; i < samples.length; i++) {
    if (samples[i]!.kwh > samples[best]!.kwh) best = i
  }
  const win = samples[best]!
  return {
    windowHours,
    label,
    ...stats,
    maxWindowStartMs: win.windowStartMs,
    maxWindowEndMs: win.windowEndMs,
  }
}
...
export function computeUsageAnalytics(
  ...
  return plan.periods.map((period) => {
    ...
    const rollingWindowSummary: RollingWindowSummaryRow[] = SUMMARY_WINDOW_HOURS.map((h) =>
      rollingWindowSummaryFromSamples(allRollingKwhSamples(slice, h), h, `${h}hr`),
    )
